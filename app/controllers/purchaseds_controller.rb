class PurchasedsController < ApplicationController
  ...
  def purchase_params
    params.require(:purchased_address).permit(:zip_code, :prefecture_id, :city, :addresses, :building, :phone_number).merge(
  ...
  def check_access
    return unless current_user.id == @product.user_id || @product.purchased.present?
